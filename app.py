...
# --- Auth por variables de entorno/Secrets (opcional) ---
...
with st.sidebar:
    ...
    extra_context = st.text_area(
        ...
        placeholder="Sites, ports, EPC candidates, known constraints, tariffs, etc."
    )
    uploaded = st.file_uploader(
        "Attach additional context (PDF/TXT/DOCX/CSV)…",
...
if "res" not in st.session_state:
    ...
    with col1:
        ...
        st.write(res.get("legal_fiscal", ""))

        st.subheader("Logistics & Infrastructure")
        st.write(res.get("logistics", ""))
    ...
    with col2:
        ...
        st.download_button(
            ...
            md.encode("utf-8"),
            file_name="QRT_Strategic_Analysis.md"
        )

        csv_bytes = risks_to_csv_bytes(res.get("risks"))
        st.download_button(
            "⬇️ Download Risks (CSV)",
            csv_bytes,
            file_name="QRT_Risks.csv",
            mime="text/csv"
        )
        ...
        if pdf_bytes:
            st.download_button(
                ...
                pdf_bytes,
                file_name="QRT_Strategic_Analysis.pdf",
